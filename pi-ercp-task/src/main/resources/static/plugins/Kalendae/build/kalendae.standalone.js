...
var Kalendae = function (targetElement, options) {
	...
	while (j--) {
		...
		do {
			if (opts.mode == 'week') {
				...
				dayNodes.push(util.make('input', {readonly:"readonly"}, $days));
			}
		} while (++i < 42);
	...
	util.addEvent($container, 'dblclick', function (event, target) {
		
		 if ( (util.hasClassName(target.parentNode, classes.days)) && util.hasClassName(target, classes.dayActive) && (clickedDate = target.getAttribute('data-date'))) {
		//DAY CLICK
			clickedDate = moment(clickedDate, opts.dayAttributeFormat).hours(12);
			if (self.publish('deSelected', self, [clickedDate]) !== false) {
				switch (opts.mode) {
					case 'multiple':
						if (util.hasClassName(target, classes.daySelected)) self.removeSelected(clickedDate);
						break;
				}
	...
	util.addEvent($container, 'click', function (event, target) {
		...
		//DAY CLICK
			...
			if (self.publish('date-clicked', self, [clickedDate]) !== false) {
				...
				switch (opts.mode) {
					case 'multiple':
						if (!util.hasClassName(target, classes.daySelected)) self.addSelected(clickedDate);
						break;
					case 'range':
	...
	draw : function draw() {
		...
		do {
			...
			do {
				...
				$span.value = day.format(opts.dayNumberFormat);
