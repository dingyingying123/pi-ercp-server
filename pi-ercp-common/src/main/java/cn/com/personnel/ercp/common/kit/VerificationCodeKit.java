 ...
 *
 */
public class VerificationCodeKit {
    ...
    private static int         lineSize      = 45;                      // 干扰线数量
    ...
    public static void getRandcode(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            e.printStackTrace();
            throw new Exception("将内存中的图片通过流动形式输出到客户端失败");
        }
    ...
    private static String drowString(Graphics g, String randomString, int i) {
        ...
        randomString += rand;
        g.translate(4 + random.nextInt(4), random.nextInt(3));
